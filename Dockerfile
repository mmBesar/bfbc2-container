...
#
# In plain words:
#   1. Download the emulator's source from the SourceForge project and check
#      it is exactly the file we expect (SHA256).
#   2. Compile it.
...
# Passed in by the workflow. The build fails if it does not match.
ARG SRC_SHA256

# Where Boost/OpenSSL live inside the build image (stage 2)
ENV LIB_BOOST_DIR="/usr/include/boost/" \
    LIB_OPENSSL_DIR="/usr/include/openssl/" \
    LIB_PATH="/usr/lib"

# unrar is in Debian "non-free", so enable that component first
...
# The project files point at the author's own library folders.
# Point them at the ones in our build image instead.
...
# ---- Stage 2: compile --------------------------------------------------------
# Old Alpine on purpose: it ships the old OpenSSL 1.0.x that this 2014-era code
# expects (BC2 clients also speak very old SSL). Only used to build; the
# result is copied out, so this does not end up in the final image.
